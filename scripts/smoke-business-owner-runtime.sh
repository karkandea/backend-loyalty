...
cleanup_phone() {
  if [[ "$PHONE_MUTATED" == '1' ]]; then
    docker exec loyalty-postgres psql -U postgres -d loyalty -q -c \
      "UPDATE \"Member\" SET \"phone\"=NULL, \"updatedAt\"=now() WHERE \"id\"='$MEMBER_ID' AND \"businessId\"='$BUSINESS_ID';" \
      >/dev/null 2>&1 || true
  fi
}
trap cleanup_phone EXIT
...
echo "==> [2/9] Owner login selects the single active business..."
...
echo "==> [3/9] Membership list and refresh rotation preserve business access..."
...
echo "==> [4/9] Owner summary reads tenant-scoped operational counts..."
...
echo "==> [5/9] Owner admin-phone route is tenant/role scoped and same-phone idempotent..."
docker exec loyalty-postgres psql -U postgres -d loyalty -v ON_ERROR_STOP=1 -q -c \
  "UPDATE \"Member\" SET \"phone\"='$SAME_PHONE', \"updatedAt\"=now() WHERE \"id\"='$MEMBER_ID' AND \"businessId\"='$BUSINESS_ID';"
PHONE_MUTATED=1
PHONE_RESPONSE="$(curl --silent --show-error --fail-with-body \
  -X PATCH \
  -H 'Content-Type: application/json' \
  -H "Authorization: Bearer $ACCESS" \
  -H 'x-device-id: qa-owner-phone-route-smoke' \
  --data "{\"phone\":\"$SAME_PHONE\"}" \
  "$BASE_URL/api/admin/member/$MEMBER_ID/phone")"
docker exec loyalty-postgres psql -U postgres -d loyalty -v ON_ERROR_STOP=1 -q -c \
  "UPDATE \"Member\" SET \"phone\"=NULL, \"updatedAt\"=now() WHERE \"id\"='$MEMBER_ID' AND \"businessId\"='$BUSINESS_ID';"
PHONE_MUTATED=0
[[ "$(printf '%s' "$PHONE_RESPONSE" | json_get data.updated)" == 'True' ]] || {
...
echo "==> [7/9] Owner creates and lists team invitation..."
...
echo "==> [8/9] Owner revokes team invitation..."
...
echo "==> [9/9] Owner logout revokes rotated refresh token..."
